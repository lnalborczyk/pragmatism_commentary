...
% ii) even in the sparse situations where the numerical boundaries of the intervals are identical, the inference that can be made from each interval is not identical,

%comes with its own pitfalls, that could easily be avoided by using parsimony instead as a guiding principle, and by relying on statistical literacy rather than misleading heuristics.
